...
global $CFG;
global $DB;
require_once $CFG->dirroot.'/plagiarism/dupli/constants.php';
require_once $CFG->dirroot.'/plagiarism/lib.php';

class plagiarism_plugin_dupli extends plagiarism_plugin
{
    /**
    ...
    public function get_links($linkarray)
    {
        ...
        // Don't show links for certain file types as they won't have been submitted to DUPLI.
        ...
        // Get dupli plugin admin config.
        static $adminconfig;
        if (empty($adminconfig)) {
            $adminconfig = self::plagiarism_dupli_admin_config();
        }

        // Is dupli plugin enabled for this module type?
        static $ismodenabled;
        if (empty($ismodenabled)) {
            $moduleconfigname = 'plagiarism_dupli_mod_'.$coursemodule->modname;
        ...
        $enabledproperty = 'plagiarism_dupli_enable';
        ...
        if (!$isinstructor && empty($modulesettings['plagiarism_dupli_allowstudentaccess'])) {
            return;
        }

        $result = $DB->get_records('plagiarism_dupli_files', ['cm' => $cmid, 'userid' => $userid, 'identifier' => $identifier],
            'lastmodified DESC', '*'
        );
        ...
        if ($submittedfile) {
            ...
            if ($submittedfile->externalid) {
                ...
                $reporturl = "$CFG->wwwroot/plagiarism/dupli/plagiarism_dupli_report.php".
                    "?cmid=$submittedfile->cm&userid=$submittedfile->userid".
                    "&externalid=$submittedfile->externalid&modulename=$coursemodule->modname";

                $similaritystr = get_string('similaritystr', 'plagiarism_dupli');
                $viewreportstr = get_string('viewreportstr', 'plagiarism_dupli');
                $clickstr = get_string('clickstr', 'plagiarism_dupli');
            ...
            $statusstr = get_string('duplistatus', 'plagiarism_dupli').': '.get_string($status, 'plagiarism_dupli');
            $output .= html_writer::tag('div', $OUTPUT->pix_icon('logo', $statusstr, 'plagiarism_dupli', ['class' => 'icon_size']).$statusstr.$score,
                ['class' => 'dupli_status ']);
        }
    ...
    */
    public function save_form_elements($data)
    {
        ...
        $default = [];
        $default['plagiarism_dupli_enable'] = $data->plagiarism_dupli_enable;
        $default['plagiarism_dupli_draftsubmit'] = isset($data->plagiarism_dupli_draftsubmit) ? $data->plagiarism_dupli_draftsubmit : 0;
        $default['plagiarism_dupli_reportgen'] = isset($data->plagiarism_dupli_reportgen) ? $data->plagiarism_dupli_reportgen : 0;
        $default['plagiarism_dupli_allowstudentaccess'] = $data->plagiarism_dupli_allowstudentaccess;

        // Get saved db settings.
        $cmid = $data->coursemodule;
        $saveddefaultvalue = $DB->get_records_menu('plagiarism_dupli_config', ['cm' => $cmid], '', 'name,value');
        ...
        foreach ($configfields as $f) {
            if (isset($default[$f])) {
                ...
                if (!isset($saveddefaultvalue[$f])) {
                    ...
                    if (!$DB->insert_record('plagiarism_dupli_config', $savedfield)) {
                        throw new moodle_exception(get_string('inserterror', 'plagiarism_dupli'));
                    }
                } else {
                    ...
                    if (!$DB->update_record('plagiarism_dupli_config', $savedfield)) {
                        throw new moodle_exception(get_string('updateerror', 'plagiarism_dupli'));
                    }
                }
            }
        }
    ...
    {
        return [
            'plagiarism_dupli_enable',
            'plagiarism_dupli_draftsubmit',
            'plagiarism_dupli_reportgen',
            'plagiarism_dupli_allowstudentaccess',
        ];
    }
    ...
    public function get_form_elements_module($mform, $context, $modulename = '')
    {
        ...
        if (
            has_capability('plagiarism/dupli:enable', $context)
            ...
            $mform->addElement(
                'header',
                'plagiarism_dupli_defaultsettings',
                get_string('coursesettings', 'plagiarism_dupli')
            );
            ...
            $mform->addElement(
                'advcheckbox',
                'plagiarism_dupli_enable',
                get_string('enable', 'plagiarism_dupli')
            );
            ...
            if ($mform->elementExists('submissiondrafts')) {
                $mform->addElement(
                    'advcheckbox',
                    'plagiarism_dupli_draftsubmit',
                    get_string('draftsubmit', 'plagiarism_dupli')
                );
                $mform->addHelpButton(
                    'plagiarism_dupli_draftsubmit',
                    'draftsubmit',
            ...
            if ($mform->elementExists('duedate')) {
                $genoptions = [
                    0 => get_string('genereportimmediately', 'plagiarism_dupli'),
                    1 => get_string('genereportonduedate', 'plagiarism_dupli'),
                ];
                $mform->addElement(
                    'select',
                    'plagiarism_dupli_reportgen',
                    get_string('reportgenspeed', 'plagiarism_dupli'),
                    $genoptions
                );
            }
            ...
            $mform->addElement(
                'advcheckbox',
                'plagiarism_dupli_allowstudentaccess',
                get_string('allowstudentaccess', 'plagiarism_dupli')
            );

            $cmid = optional_param('update', null, PARAM_INT);
            $savedvalues = $DB->get_records_menu('plagiarism_dupli_config', ['cm' => $cmid], '', 'name,value');

            if (count($savedvalues) > 0) {
                $mform->setDefault(
                    'plagiarism_dupli_enable',
                    isset($savedvalues['plagiarism_dupli_enable']) ? $savedvalues['plagiarism_dupli_enable'] : 0
                );

                $draftsubmit = isset($savedvalues['plagiarism_dupli_draftsubmit']) ?
                    $savedvalues['plagiarism_dupli_draftsubmit'] : 0;

                $mform->setDefault('plagiarism_dupli_draftsubmit', $draftsubmit);
                if (isset($savedvalues['plagiarism_dupli_reportgen'])) {
                    $mform->setDefault('plagiarism_dupli_reportgen', $savedvalues['plagiarism_dupli_reportgen']);
                }
                if (isset($savedvalues['plagiarism_dupli_allowstudentaccess'])) {
                    $mform->setDefault(
                        'plagiarism_dupli_allowstudentaccess',
                        $savedvalues['plagiarism_dupli_allowstudentaccess']
                    );
                }
            } else {
                $mform->setDefault('plagiarism_dupli_enable', false);
                $mform->setDefault('plagiarism_dupli_draftsubmit', 0);
                $mform->setDefault('plagiarism_dupli_reportgen', 0);
                $mform->setDefault('plagiarism_dupli_allowstudentaccess', 0);
            }
    ...
    public function print_disclosure($cmid)
    {
        ...
        // Check if dupli plugin is enabled for this module.
        $moduleenabled = $this->is_plugin_configured('mod_'.$cm->modname);
        if (empty($modulesettings['plagiarism_dupli_enable']) || empty($moduleenabled)) {
            return '';
        }

        $plagiarismsettings = (array) get_config('plagiarism_dupli');

        $isuseragreed = $this->is_user_eula_accepted($USER->id);

        if (!$isuseragreed) {
            if (isset($plagiarismsettings->plagiarism_dupli_studentdisclosure)) {
                $studentdisclosure = $plagiarismsettings->plagiarism_dupli_studentdisclosure;
            } else {
                $studentdisclosure = get_string('studentdisclosuredefault', 'plagiarism_dupli');
            }
        } else {
            $studentdisclosure = get_string('studentdagreedtoeula', 'plagiarism_dupli');
        }
     ...
     */
    public static function plagiarism_dupli_admin_config()
    {
        return get_config('plagiarism_dupli');
    }

    /**
     * Check if plugin has been enabled with dupli .
     *
     * @return bool whether the plugin is enabled for dupli
     **/
    public function is_plugin_enabled()
    {
        $config = self::plagiarism_dupli_admin_config();
    ...
    /**
     * Check if plugin has been configured with dupli account details.
     *
     * @return bool whether the plugin is configured for dupli
     **/
    public function is_plugin_configured($modulename = null)
    {
        $config = self::plagiarism_dupli_admin_config();

        if (empty($config->plagiarism_dupli_secretkey)) {
            return false;
        }

        if ($modulename != null) {
            $moduleconfigname = 'plagiarism_dupli_'.$modulename;
    ...
    {
        $pluginconfig = get_config('plagiarism_dupli', 'plagiarism_dupli_'.$modulename);

        return $pluginconfig;
    }

    /**
     * Get the dupli settings for a module.
     *
     ...
     *
     * @return array of dupli settings for a module
     */
    public function get_settings($cmid = null, $uselockedvalues = true)
    {
        global $DB;
        $defaults = $DB->get_records_menu('plagiarism_dupli_config', ['cm' => null], '', 'name,value');
        $settings = $DB->get_records_menu('plagiarism_dupli_config', ['cm' => $cmid], '', 'name,value');
    ...
    {
        global $DB;
        $id = $DB->get_field('plagiarism_dupli_users', 'id', (['userid' => $userid]));
        ...
        if ($id) {
            $defaultfield->id = $id;
            $DB->update_record('plagiarism_dupli_users', $defaultfield);
        } else {
            $DB->insert_record('plagiarism_dupli_users', $defaultfield);
        }
    ...
    public function is_user_eula_accepted($userid)
    {
        ...
        $user = $DB->get_record('plagiarism_dupli_users', ['userid' => $userid]);
        if (!$user || !isset($user)) {
            return false;
        }
    ...
    private function create_new_submission($cm, $userid, $identifier, $submissiontype, $studentread, $scheduledscandate)
    {
        ...
        if (!$fileid = $DB->insert_record('plagiarism_dupli_files', $plagiarismfile)) {
            plagiarism_dupli_activitylog('Insert record failed (CM: '.$cm->id.', User: '.$userid.')', 'PP_NEW_SUB');
            $fileid = 0;
        }
    ...
    private function reset_submission($cm, $userid, $identifier, $currentsubmission, $submissiontype)
    {
        ...
        if (!$DB->update_record('plagiarism_dupli_files', $plagiarismfile)) {
            plagiarism_dupli_activitylog('Update record failed (CM: '.$cm->id.', User: '.$userid.')', 'PP_REPLACE_SUB');
        }
    }
     ...
     */
    public function plagiarism_dupli_retrieve_successful_submissions($author, $cmid, $identifier)
    {
        global $CFG, $DB;
    ...
    /**
     * Queue submissions to send to dupli.
     *
     * @param $cm
     * @param $author
     * @param $submitter
    ...
    public function queue_submission($cm, $author, $submitter, $identifier, $submissiontype, $itemid = 0, $eventtype = null, $scheduledscandate = 0)
    {
        ...
        $plagiarismfiles = $this->plagiarism_dupli_retrieve_successful_submissions($author, $cm->id, $identifier);
        ...
        $studentread = isset($settings['plagiarism_dupli_allowstudentaccess']) ? $settings['plagiarism_dupli_allowstudentaccess'] : 0;
        // Get module data.
        ...
        switch ($submissiontype) {
            ...
            case 'text_content':
                ...
                $previoussubmissions = $DB->get_records_select('plagiarism_dupli_files',
                ...
                if ($previoussubmission) {
                    ...
                    if (in_array($previoussubmission->statuscode, ['success', 'error'])
                        ...
                        // Replace submission in the specific circumstance where dupli can accommodate resubmissions.
                    ...
                    $typefield = ($CFG->dbtype == 'oci') ? ' to_char(submissiontype) ' : ' submissiontype ';
                    if ($previoussubmission = $DB->get_record_select('plagiarism_dupli_files',
                        ...
                        // Replace submission in the specific circumstance where dupli can accomodate resubmissions.
            ...
            case 'forum_post':
            case 'quiz_answer':
                if ($previoussubmissions = $DB->get_records_select('plagiarism_dupli_files',
        ...
        if (!$acceptanyfiletype && $submissiontype == 'file') {
            ...
            if (!in_array('.'.$fileext, PLAGIARISM_ACCEPTED_DUPLI_FILE_EXTS)) {
                $errorcode = 4;
            }
        }
        ...
        $errormsg = ($errorcode != null) ? get_string('errorcode'.$errorcode, 'plagiarism_dupli') : null;
    ...
    public function event_handler($eventdata)
    {
        ...
        if ($cm->modname == 'assign') {
            $plagiarismsettings['plagiarism_dupli_draftsubmit'] = (isset($plagiarismsettings['plagiarism_dupli_draftsubmit'])) ? $plagiarismsettings['plagiarism_dupli_draftsubmit'] : 0;
        }

        // Either module not using dupli or dupli not being used at all so return true to remove event from queue.
        if (empty($plagiarismsettings['plagiarism_dupli_enable']) || empty($moduleenabled)) {
            return true;
        }
        ...
        if ($plagiarismsettings['plagiarism_dupli_reportgen'] == 1) {
        ...
        if ($moduledata->submissiondrafts && $plagiarismsettings['plagiarism_dupli_draftsubmit'] == 1 &&
        ...
        */
        ...
        if ($eventdata['eventtype'] == 'quiz_submitted') {
            ...
            foreach ($attempt->get_slots() as $slot) {
                ...
                foreach ($files as $file) {
                    // Queue file for sending to dupli.
                    $identifier = $file->get_pathnamehash();
                    $result = $this->queue_submission(
        ...
        // Queue text content and forum posts to send to dupli.
        ...
        // Queue files to submit to dupli.
        $result = $result && true;
        if (!empty($eventdata['other']['pathnamehashes'])) {
            foreach ($eventdata['other']['pathnamehashes'] as $pathnamehash) {
                ...
                if (!$file) {
                    plagiarism_dupli_activitylog('File not found: '.$pathnamehash, 'PP_NO_FILE');
                    $result = true;
                    continue;
                } else {
                    try {
                        ...
                        plagiarism_dupli_activitylog('File content not found: '.$pathnamehash, 'PP_NO_FILE');
                        mtrace($e);
    ...
    */
                                    ...
                                    $submissiontype, $attempt, $studentread = 1, $scheduledscandate = 0, $errorcode = null, $errormsg = null)
    {
        ...
        if ($submissionid != 0) {
            if (!$DB->update_record('plagiarism_dupli_files', $plagiarismfile)) {
                plagiarism_dupli_activitylog('Update record failed (CM: '.$cm->id.', User: '.$userid.') - ', 'PP_UPDATE_SUB_ERROR');
            }
        } else {
            if (!$DB->insert_record('plagiarism_dupli_files', $plagiarismfile)) {
                plagiarism_dupli_activitylog('Insert record failed (CM: '.$cm->id.', User: '.$userid.') - ', 'PP_INSERT_SUB_ERROR');
            }
        }
    ...
    public function save_errored_submission($submissionid, $attempt, $errorcode)
    {
        ...
        if (!$DB->update_record('plagiarism_dupli_files', $plagiarismfile)) {
            plagiarism_dupli_activitylog('Update record failed (Submission: '.$submissionid.') - ', 'PP_UPDATE_SUB_ERROR');
        }
    ...
    /**
     * Handle Scheduled Task to Send Queued Submissions to dupli.
     */
    public function send_queued_submissions()
    {
        ...
        // Don't attempt to call  if a connection to dupli could not be established.

        $currentdate = strtotime('now');
        $queueditems = $DB->get_records_select('plagiarism_dupli_files', '(statuscode = ? OR statuscode = ?) AND duedatescan < ?  ',
            [PLAGIARISM_DUPLI_QUEUED_STATUS, PLAGIARISM_DUPLI_PENDING_STATUS, $currentdate], 'lastmodified', '*', 0, PLAGIARISM_DUPLI_CRON_SUBMISSIONS_LIMIT);

        // Submit each file individually to dupli.
        foreach ($queueditems as $queueditem) {
            ...
            if (empty($cm)) {
                ...
                // Output a message in the cron for failed submission to dupli.
                $outputvars = new stdClass();
                $outputvars->id = $queueditem->id;
                $outputvars->cm = $queueditem->cm;
                $outputvars->userid = $queueditem->userid;

                plagiarism_dupli_activitylog(get_string('errorcode12', 'plagiarism_dupli', $outputvars), 'PP_NO_COURSE');
                continue;
            }
            ...
            // Clean up old dupli submission files.
            ...
            switch ($queueditem->submissiontype) {
                ...
                case 'text_content':
                    ...
                    if ($queueditem->submissiontype == 'file') {
                        ...
                        if (!$file) {
                            plagiarism_dupli_activitylog('File not found for submission: '.$queueditem->id, 'PP_NO_FILE');
                        ...
                        try {
                            ...
                            plagiarism_dupli_activitylog('File content not found on submission: '.$queueditem->identifier, 'PP_NO_FILE');
                            mtrace($e);
                ...
                case 'quiz_answer':
                    ...
                    try {
                        ...
                        plagiarism_dupli_activitylog(get_string('errorcode14', 'plagiarism_dupli'), 'PP_NO_ATTEMPT');
                        $errorcode = 14;
                        break;
                    }
                    foreach ($attempt->get_slots() as $slot) {
            ...
            // Call dupli API to check content
            require_once __DIR__.'/classes/plagiarism_dupli_api_provider.php';
            ...
            $apiprovider = new plagiarism_dupli_api_provider($apikey, $apitoken);
            ...
            if ($response_data != null || !empty($response_data)) {
                ...
                $plagiarismfile->reporturl = isset($response_data->report_url) ? $response_data->report_url : PLAGIARISM_DUPLI_API_BASE_URL.'/plag/scan/mdl/report/'.$response_data->hash;
                $DB->update_record('plagiarism_dupli_files', $plagiarismfile);
            }
            ...
            mtrace(get_string('cronsubmittedsuccessfully', 'plagiarism_dupli', $outputvars));
        }
    }
    ...
    private function clean_old_submissions($cm, $userid, $itemid, $submissiontype, $identifier)
    {
        ...
        if ($submissiontype == 'file') {
            ...
            if ($moodlefiles = $DB->get_records_select('files', ' component = ? AND userid = ? AND source IS NOT null AND '.$itemidsinsql,
                ...
                $typefield = ($CFG->dbtype == 'oci') ? ' to_char(submissiontype) ' : ' submissiontype ';
                $oldfiles = $DB->get_records_select('plagiarism_dupli_files', ' userid = ? AND cm = ? '.
        ...
        if (!empty($deletestr)) {
            if (!$DB->delete_records_select('plagiarism_dupli_files', $deletestr, $deleteparams)) {
    ...
    /**
     * Creates a temp file for submission to dupli, uses a random number suffixed with the stored filename.
     *
    ...
    {
        $tempdir = make_temp_directory('plagiarism_dupli');
        ...
        do {
            if ($tries == 5) {
                throw new invalid_dataroot_permissions('dupli plagiarism plugin temporary file cannot be created.');
            }
            ++$tries;
 ...
 * @return bool Returns true by default. Extend functionality as needed.
 */
function plagiarism_dupli_event_file_uploaded($eventdata)
{
    $result = true;
 ...
 * @return bool Returns true by default. Extend functionality as needed.
 */
function plagiarism_dupli_event_files_done($eventdata)
{
    $result = true;
 ...
 * @return bool Returns true by default. Extend functionality as needed.
 */
function plagiarism_dupli_event_mod_created($eventdata)
{
    $result = true;
    //a dupli module has been created - this is a generic event that is called for all module types
 ...
 * @return bool Returns true by default. Extend functionality as needed.
 */
function plagiarism_dupli_event_mod_updated($eventdata)
{
    $result = true;
 ...
 * @return bool Returns true by default. Extend functionality as needed.
 */
function plagiarism_dupli_event_mod_deleted($eventdata)
{
    $result = true;
 ...
 */
function plagiarism_dupli_activitylog($string, $activity)
{
    global $CFG;
...
/**
 * Add the Duplichecker settings form to an add/edit activity page.
 *
 ...
 * @var mixed $course
 */
function plagiarism_dupli_coursemodule_standard_elements($formwrapper, $mform)
{
    $dupli_plugin = new plagiarism_plugin_dupli();
    $course = $formwrapper->get_course();
    $context = context_course::instance($course->id);
    $modulename = $formwrapper->get_current()->modulename;

    $dupli_plugin->get_form_elements_module(
        $mform,
        $context,
...
/**
 * Handle saving data from the Duplichecker settings form.
 *
 * @param stdClass $data
 * @param stdClass $course
 */
function plagiarism_dupli_coursemodule_edit_post_actions($data, $course)
{
    $dupliplugin = new plagiarism_plugin_dupli();
    $dupliplugin->save_form_elements($data);
}
